...
현재 셸에 키를 설정하면 이후 그 셸에서 실행하는 프로그램이 상속받습니다.
각 프로그램은 공통 URL과 `OPENAI_API_KEY`를 읽도록 설정하세요.
이미 실행 중인 프로그램이나 다른 SSH 터미널에는 적용되지 않습니다.

```csh
# csh/tcsh
eval `codex-rate-proxy env -u hoony --shell csh`
```

```bash
# Bash
eval "$(codex-rate-proxy env -u hoony --shell bash)"
```

다른 변수명을 사용하는 프로그램은 `--var MY_LLM_KEY`를 추가하세요.
`env` 명령은 INI나 게이트웨이 실행 없이 등록된 키를 읽습니다.
기존에 같은 이름의 환경변수가 있으면 교체하며, 오류가 나면 기존 값은 유지됩니다.
명령의 오류 메시지를 확인하세요. `eval`의 종료 코드만으로 키 설정 성공을 판단하지 마세요.

출력은 키를 복원할 수 있는 셸 명령이므로 로그·파일에 저장하거나 공유하지 마세요.
직접 터미널에 출력하는 것은 거부합니다. 셸의 명령 추적(`set -x`, csh의 `echo`/`verbose`)을
활성화한 상태에서는 사용하지 마세요. csh/tcsh는 서버의 `/usr/bin/printf`를 사용합니다.
